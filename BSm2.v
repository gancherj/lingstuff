...
(* Mapping of syntactic categories to semantic types. *)
  Fixpoint interp (c : Cat) : Type :=
    ...
    | Struct a b => interp a * (interp a -> interp b)
...
Definition john : DP := (e "john").
Definition mary : DP := (e "mary").
Definition ben : DP := (e "ben").
Definition alice : DP := (e "alice").
Definition ari : DP := (e "ari").
...
(* Collapse tower at S. The result is the scope result a. *) 
...
(* Tower application \ composition. The arrow points at the
downstairs argument. *)
...
Definition mkTV (s : string) : TV := fun (y : E) (x : E) => ((eet s) x y).

(* This puts the subject first in the atomic formula (love john mary) *)
Definition loves : TV := mkTV "love".
Definition kissed : TV := mkTV "kiss".

(*
Eval compute in loves.
  = fun y x : E => eet "love" x y
     : TV
*)
...
(* 
Use "shift space" to covert latex to unicode.
\blacktriangleleft \u25c0
\blacktriangleright \u25b7
\triangleright \u25b7
\triangleleft \u25c1
\LeftTriangleBar \u29cf   Hard to read
\Gfr \U0001d50a
Notation "x \u25c2 y"
*)
...
(* 
   ...
   lifted to a tower in order to combine with the quantifiers,
   which are towers.
...
Eval compute in (lift loves) |> someone.
